...
// ── Rotas estáticas (SEMPRE antes das dinâmicas /:id) ──────────
...
router.post('/train', requireAuth, requireRole('admin'),
  ...
  (req, res) => {
    ...
    db.prepare(`
      ...
      VALUES (?,?,?,?,?,?,?,?,?)
    `).run(modelId, name, version, dataset_id, architecture, epochs, 'training', notes || null, req.user.id);
...
// PATCH /api/models/jobs/:jobId/progress
router.patch('/jobs/:jobId/progress', requireAuth, requireRole('admin'), (req, res) => {
  ...
  if (log_line) {
    ...
    vals.push(lines.slice(-500).join('\n'));
  }
...
// ── Rotas dinâmicas /:id (SEMPRE depois das estáticas) ─────────
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  if (model.metrics_json) try { model.metrics = JSON.parse(model.metrics_json); } catch {}
  res.json(model);
});

// PATCH /api/models/:id
router.patch('/:id', requireAuth, requireRole('admin'), (req, res) => {
  const ALLOWED = ['status', 'map50', 'map95', 'precision', 'recall', 'model_path', 'metrics_json', 'notes'];
...
router.post('/:id/deploy', requireAuth, requireRole('admin'), (req, res) => {
  ...
  if (model.status !== 'completed')
    return res.status(400).json({ error: 'Apenas modelos com status completed podem ser deployados.' });
...
router.delete('/:id', requireAuth, requireRole('admin'), (req, res) => {
  ...
  if (model.model_path && fs.existsSync(model.model_path)) {
    try { fs.unlinkSync(model.model_path); } catch {}
  }
